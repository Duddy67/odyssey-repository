...
class OrderHelper
{
  public static function storeOrder($travel, $addons, $settings)
  {
    ...
    $orderDetails = JLayoutHelper::render('order_details', array('travel' => $travel, 'addons' => $addons, 'settings' => $settings));
    $db = JFactory::getDbo();
    ...
    $columns = array('order_nb','customer_id','payment_status','order_status',
		     ...
		     'currency_code','rounding_rule','digits_precision','overlapping','created');
    ...
    $values = $db->quote('undefined').','.(int)$user->get('id').','.$db->quote('pending').','.$db->quote('pending').','.
	      ...
	      $db->quote($dptDate).','.(int)$travel['nb_psgr'].','.$db->quote($orderDetails).','.
	      $db->quote($settings['currency_code']).','.$db->quote($settings['rounding_rule']).','.
	      (int)$settings['digits_precision'].','.(int)$travel['overlapping'].','.$nowDate;
  ...
  public static function storeAddons($addons, $orderId)
  {
	...
	$values2[] = (int)$orderId.','.(int)$addon['step_id'].','.(int)$addon['addon_id'].','.
		    (int)$addonOption['addon_option_id'].','.$db->quote($addonOption['name']).','.$db->quote($addonOption['code']).','.(float)$addonOption['price'];
      }
    }
    ...
    if(!empty($values2)) {
      $columns = array('order_id','step_id','addon_id','addon_option_id','name','code','price');
      $query->clear();
