...
<form action="<?php echo JRoute::_('index.php?option=com_users&view=users&layout=modal&tmpl=component&groups=' . $input->get('groups', '', 'BASE64') . '&excluded=' . $input->get('excluded', '', 'BASE64'));?>" method="post" name="adminForm" id="adminForm">
  ...
  <?php else : ?>
	  <table class="table table-striped table-condensed">
		...
		<td>
	    <a class="pointer" style="color:#025a8d;" onclick="if(window.parent) window.parent.<?php echo $this->escape($function);?>(<?php echo $item->id; ?>, '<?php echo $this->escape(addslashes($item->name)); ?>', '<?php echo $idNb; ?>', 'recipient');" ><?php echo $this->escape($item->name); ?></a>
		</td>
		<td align="center">
