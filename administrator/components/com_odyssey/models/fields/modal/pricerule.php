...
class JFormFieldModal_Pricerule extends JFormField
{
  ...
  protected function getInput()
  {
    ...
    if($allowClear && !$scriptClear) {
	    ...
	    $script[] = '		document.getElementById(id + "_name").value = "'.htmlspecialchars(JText::_('COM_ODYSSEY_SELECT_A_PRICERULE', true), ENT_COMPAT, 'UTF-8').'";';
    ...
    $modalOption = '';
    if($this->form->getName() == 'com_odyssey.coupon') {
      $modalOption = '&modal_option=coupon_only';
    }

    // Setup variables for display.
    $html = array();
    $link = 'index.php?option=com_odyssey&amp;view=pricerules&amp;layout=modal&amp;tmpl=component'.$modalOption.'&amp;function=jSelectPricerule_'.$this->id; 
    ...
    if((int) $this->value > 0) {
      ...
      $query = $db->getQuery(true)
	      ...
	      ->from($db->quoteName('#__odyssey_pricerule'))
    ...
    if(empty($title)) {
      $title = JText::_('COM_ODYSSEY_SELECT_A_PRICERULE');
    }
    ...
    $html[] = '<a class="modal btn hasTooltip" title="'.JHtml::tooltipText('COM_ODYSSEY_CHANGE_PRICERULE').'"  href="'.$link.'&amp;'.JSession::getFormToken().'=1" rel="{handler: \'iframe\', size: {x: 800, y: 450}}"><i class="icon-file"></i> '.JText::_('JSELECT').'</a>';
