...
class OdysseyModelCoupons extends JModelList
{
  ...
  protected function getListQuery()
  {
    ...
    $query->select($this->getState('list.select', 'c.id,c.name,c.code,c.created,'.
				   'c.published,c.created_by,c.checked_out,c.checked_out_time'));

    $query->from('#__odyssey_coupon AS c');

    //Get the user name.
    $query->select('u.name AS user');
    $query->join('LEFT', '#__users AS u ON u.id = c.created_by');

    //Get the price rule name.
    $query->select('p.name AS prule_name');
    $query->join('LEFT', '#__odyssey_pricerule AS p ON p.id = c.prule_id');
